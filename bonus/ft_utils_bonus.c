...
/*   Updated: 2024/05/09 11:35:59 by eismail          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	element_check(char **s)
{
	...
	if (element_count(s, 'T') < 1)
		map_error(s, "the map must contain at least one coin 'T'\n");
	...
	while (s[i])
	{
		...
		while (s[i][j])
		{
			if (s[i][j] != 'P' && s[i][j] != 'C' && s[i][j] != 'E' 
				&& s[i][j] != '1' && s[i][j] != '0' && s[i][j] != 'T')
				map_error(s, "Ony 'P' 'E' 'C' '1' '0' 'T'\n");
			if (s[i][j] == c)
				count++;
			j++;
		}
